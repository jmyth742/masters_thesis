...
Currently there exists three primary methods of multiple access (there are many variations on these). Time-division, Frequency-division, Code-division. The following points outline the basic methodology behind these access technologies, paying attention to how they access the medium. 
\newline \textbf{TDMA} - Time Division multiple access,  relies on signals which have been digitised and is based on time division multiplexing \cite{tdma_info}. TDMA in essence allocates different time slots for the users of a channel to transmit their data. In TDMA systems, the spectrum of the available radio is divided into user specific time slots, only the allocated user can transmit on that respective slot \cite{tdma_info}.
\newline \textbf{CDMA} - Code division multiple access uses spread spectrum techniques, meaning each user is not allocated a particular frequency, instead they utilise the full available spectrum. CDMA transmissions are encoded with pseudo-random digital sequences, which are separated by orthogonal codes\cite{cdma_info}. The receiver knows the user specific code sequence and uses said sequence to decode the received signal, this is possible because the cross correlation between different users is small. This can be achieved as the bandwidth of the code signal is chosen to be vastly larger than the information signal, the encoding spreads the signal across the entire spectrum \cite{cdma_info}. \newline
\textbf{FDMA} - Frequency division multiple access (FDMA) assigns users an individual frequency band of the entire available wireless channel frequency. Each users' channel is allocated on demand when entering the cell region \cite[Section...]{fdma_info}. All active adjacent frequencies in a cell are divided by a guard band to reduce cross talk between channels. FDMA is based on frequency division multiplexing and each user has a separate frequency for both uplink and downlink \cite{fdma_info}.\newline  
...
% \subsubsection{System Model}\label{sys_mod} 
...
Now we should present and discuss the system model for TBMA. As we have set the scene is the previous paragraph about how the different aspects of the systems model come together. In waveform design, a crucial observation is that the estimator doesn't actually need to know the exact values of $X_1 \dots X_n $, to achieve its best performance \cite{tbma}. In reality, if each of these nodes could deliver a sufficient statistic with their respective transmissions, then there is actually no loss of information. An example of such a statistic would be the empirical measure , i.e the type:

\begin{align}
    \tilde{p} = \frac{1}{n} (N_1 ,\dots, N_k)
\end{align}
where $N_j =\sum_{i=1}^n 1(X_i = j)$ is the number of sensor nodes in the network that observe $j^4$. If we let $u_1,\dots,u_k$ be $k$ orthonormal waveforms, and then setting $s_{i,X_i} = \sqrt{E}u_{X_i}$, which corresponds to letting every node observing $j$ transmit $u_j$ with energy $E$. We then arrive at the signal at the fusion centre which is presented as:

\begin{align}
    \boldsymbol{z} = \sum_{i=1}^n h_i  \sqrt{E}u_{X_i} + \boldsymbol{w} \label{eq:system_model}
\end{align}

Where $w$ is the channel noise, which is assumed to be white proper-complex Gaussian noise $\mathcal{CN}(0, \sigma^2 \boldsymbol{I}_N)$ and with power $\sigma^2/2$. Where, $h$ is the actual channel itself and the rest of the terms are as described above.  This becomes easier to understand when we consider an identical channel, where all $h_i$ terms are considered as equal to 1. Which then would simplified

\begin{align}
    \boldsymbol{z} = \sum_{j=1}^k \sqrt{E}N_j u_{j} + \boldsymbol{w} \label{eq:simplified_system_model}
\end{align}

We see that the received signal contains a noisy version of the empirical measure once we have applied a matched filter of $u_1,\dots,u_k$ and scaled by $1\sqrt{E}n$.

%
\begin{align}
    \boldsymbol{z} &= K \tilde{\boldsymbol{p}} + \boldsymbol{v}, 
\end{align}
%
where $\tilde{\boldsymbol{p}} = \frac{1}{K}[N_1, \ldots, N_R]$ is the \emph{empirical measure} or \emph{type} with
\begin{align}
    N_j = \sum_{k \in \mathcal{K}}\mathbb{1}(X_k = j)
\end{align}
and $\boldsymbol{w}^i \sim \mathcal{CN}(0, {\sigma^2 \boldsymbol{I}_R})$, which is the complex Gaussian noise.
%
...
Now we move onto an application of \ac{TBMA}, if we consider the above description, looking at how the data model should then be. We consider a set of sensor nodes $n$, who observes an \ac{i.i.d} event, $X$, where  $X \in\{1,0\}^R$.
Nodes in the system can be prone to errors or may be out of scope of the event, thus we have a measurement vector of ${x \in \{0, 1\}}$ which is obtained by the following :
...
% 
In this section, we will go into greater detail about the system models presented in \cref{eq:system_model} and \cref{eq:simplified_system_model}, which will given context in the LTE frame work. We will address a couple of use cases for \ac{tbma}, looking at both CQI and ACK/NACK feedback.
Lastly, how the parameter estimation of the TBMA is presented for both identical and fading channels. Finally, potential LTE transmission schemes are presented and discussed with applications within the LTE framework. 

First, we  consider a set of User Equipment (UE) or nodes, where ${k}$ of ${K} = K$, these $K$ nodes observes various data outcomes which are conditionally \ac{i.i.d}. Each UE jointly observes data, $X_1, \ldots, X_K$, given a parameter $\theta$. It is assumed that every observation  ${X_k \in \{1, \ldots, R\}}$, takes only discrete values with a probability mass function (PMF) ${p_{\theta} = [p_{\theta}(1), \ldots, p_{\theta}(R)]}$, where each $p_{\theta}$, corresponds to a certain real world value. The PMF belongs to a family, $\{p_{\theta}: \theta \in \Theta\}$ where $\Theta \subset \mathbb{R}$ is the parameter space.
...
We can see now how this generic \ac{TBMA} model can be built up to incorporate a variety of different real world cases. Under this new adaptation for a specific use case we would arrive to a LTE CQI specific system model for realising \ac{TBMA}. 


\begin{align}
    \boldsymbol{y} = \sum_{k \in \mathcal{K}} h_k \boldsymbol{s}_{CQI_k} + \boldsymbol{w}, \label{eq:system_model_cqi}
\end{align}
...
% the have definntion to the matrix 
...
With channel coefficients drawn \ac{i.i.d}. between sensors and observation intervals $h^i_k \sim \mathcal{CN}(0, 1)$, the output of the matched filter is a vector of random variables, where the pdf of the $j$-th element is given by $p_{\mathbf{z}}(\boldsymbol{z}^i[j]; N_j)$, with  
%
\begin{align}
...
This section details the results, analysis and evaluation of both the numerical simulation and also the \ac{POC} in the srsLTE SDR environment.
