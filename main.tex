...
I would like to give a special thank you to the Fraunhofer HHI, for supporting my work during this thesis and in particular to both Dennis Wieruch and Johannes Dommel, for their supervision during this time. I would like to thank EIT Digital for creating such an insightful and wonderful experience. Finally i would like to thank my family for their support when times were tough.
...
\begin{figure}[H]
    ...
    keepaspectratio,]
    {Figure_1.png}
    \caption{ Two groups of UEs, transmitting Quantized SNR values}
    \label{fig:snr_cqi_plot}
\end{figure}


In the case of the quantized SNR values, we can exploit this common relationship between the observed data, which results in the underlying physical process. For the given example in this section it is the channel gain that incorporates the path loss, which we can see in \cref{fig:pmfs}. Clearly given what could be the received empirical measure of the data depicted in \cref{fig:pmfs}. We can distinguish between two users groups and based on their \ac{SNR}, we could clearly see that likelihood is that they are close to each other and based on their quantified SNR, thus recommend a suitable efficient \ac{CQI} value. In an ideal world we want to find the true empirical of the observations across all the users in a group. 
...
\begin{figure}[H]
    ...
    keepaspectratio,]
    {Figure_2.png}
    \caption{UE groups Empirical Measure}
    \label{fig:pmfs}
\end{figure}
...
Now we should present and discuss the system model for TBMA. In waveform design, a crucial observation is that the estimator doesn't actually need to know the exact values of $X_1 \dots X_k $, to achieve its best performance \cite{tbma}. In reality, if each of these nodes could deliver a sufficient statistic with their respective transmissions, then there is actually no loss of information. An example of such a statistic would be the empirical measure , i.e the type:
...
Until now we have presented a generic \ac{TBMA} model and looked a feedback mechanisms in LTE. First, we would point on a notable limitation of deriving the empirical distribution of the data. Next, we apply the system models presented in \cref{eq:system_model}, \cref{eq:simplified_system_model} and \cref{eq:system_model_retrans} in context of the LTE framework. We will address two use cases for \ac{TBMA}, looking at both CQI and ACK/NACK feedback and how \ac{TBMA} can be adopted to serve these mechanisms. Lastly, how the to estimation empirical distribution of the feedback metric through TBMA in LTE.

\subsubsection{Limitations of TBMA in LTE}
When estimating the empirical measure thus trying to fit a distribution to the received data, there are some limitation of this approach when considering realworld applications. Deriving the underlying distribution (when/if discovered) we need to consider some practical limitations. To illustrate with an example, lets again consider the scenario of multiple users perform \ac{TBMA} for an arbitrary value. For example, a value of interest has a range of $0 \dots 15$. We may discover that in the wild the underlying distribution of this values could end up being modelled as e.g. Poisson, which can take on values up to $\infty$. 
...
\begin{align}
    \boldsymbol{y} = \sum_{k=1}^K h_k^l \boldsymbol{s}_{CQI_k^l} + \boldsymbol{w^l}, \label{eq:system_model_cqi}
\end{align}
...
With the architectural aspect of \ac{LTE} discussed in the previous sections we could potentially suggest a few modifications to the its structure to incorporate \ac{TBMA}. First and foremost, the physical layer of LTE could be adopted to encode and decode the raw signals needed to access the physical channel over \ac{TBMA}. As with the physical encoding and decoding of signals done on the physical channels discussed previously. We could introduce the access method \ac{TBMA}, which could be triggered on the uplink  controlled by the \ac{MAC} layer when a multicast transmission is received. This would prompt for example, the \ac{PHY} layer of that particular \ac{UE} to who could be a memeber of a group transmission to encode their 
